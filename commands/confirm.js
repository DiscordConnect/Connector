exports.run = async (Discord, client, message, args, db, config, cooldown) => {
  ...
  var stmt = db.prepare('SELECT * FROM reports WHERE report_id = (?)')
  stmt.get(report_id, async (err, report) => {
    ...
    db.run(`UPDATE reports SET confirmations = "${report.confirmations}, ${message.author.tag}" WHERE report_id = ${report_id}`)
    db.all(`SELECT * FROM reported_users WHERE report_id = ${report_id}`, async (err, reported) => {
      if (err) return message.reply("An error has occured while processing this command!")
